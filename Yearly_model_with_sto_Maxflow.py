...
directory = os.getcwd()
os.chdir(directory)
def run_model_sto_max(supplycap, supplycost, arccap, arccost, arcmin, arcmax, dmd, tariff_surc,
                      sto_par_df_db, inj_cost_db, ext_cost_db, Forward12m = False):

    # Add one more column for every dataframe

    dataframelist = [supplycap,supplycost,arccap,arccost,arcmin,arcmax,dmd,
    ...
    arc_max_flow = _get_restricted_data(arcmax,valid_rules=arc_valid_rules1)[0]
    arc_max_flow.max_flow = arc_max_flow.max_flow.astype(float)
    ...
    dflst = [supply,arcs,arc_min_flow,arc_max_flow,demand,
              sto_par_df,inj_cost,ext_cost]
    for df in dflst:
        df = df.sort_values(['date']).copy()

    #--------------------------------------------------------------------------------

    # by this point we have all our input data (supply, arcs, minflows, maxflow, demand, sto_par_df, inj_cost, ext_cost)
    results = namedtuple('NeMo_results', ['production', 'prices','flows', 'solver_info','gas_invt','gas_inj','gas_ext'])
    ...
    if Forward12m == False:
        ...
        for i in range(0,len(actual_dates),12):
            ...
            solution = solve_network(ID,d_range,supply,arcs,arc_min_flow,arc_max_flow,demand,sto_par_df,inj_cost,ext_cost,init_sto_first)
        ...
        for date in act_dates:
            ...
            if date != act_dates[-1]:
                ...
                solution = solve_network(dateID,dateRange,supply,arcs,arc_min_flow,arc_max_flow,demand,sto_par_df,inj_cost,ext_cost,init_sto_first)
            ...
            # keep the solution for the whole year
                ...
                last_solve = solve_network(dateID,dateRange,supply,arcs,arc_min_flow,arc_max_flow,demand,sto_par_df,inj_cost,ext_cost,init_sto_first)
...
#---------------------
    return full_solved_supply,full_solved_demand,full_solved_arcs, solver_status, full_solved_inj, full_solved_ext,full_solved_stoInvt

    #-WF_20181008-- Step2: Add storage variables and change the loop--- 
def solve_network(dateID,dates,supply,arcs,arc_min_flow,arc_max_flow,demand,sto_par_df,inj_cost,ext_cost,init_sto_first):
    """
    ...
    # WF_arc_max_flow
    arc_max_flow_t = arc_max_flow[(arc_max_flow['date'] >= first) & (arc_max_flow['date'] <= end)].copy()
    ...
    # objective function
    prob += lpSum(eqn_cost_s) + lpSum(eqn_cost_hh) + lpSum(eqn_cost_inj) + lpSum(eqn_cost_ext) + lpSum(eqn_cost_storing_1), 'Sum_Costs'
    ...
    # max flow_Winnie
    max_hh = arc_max_flow_t.set_index(['from_hub', 'to_hub','str_date'])[
            'max_flow'].to_dict()  # maxflow doesn't have tranche 
    
    for (hin, hout,dt) in max_hh:
        id_hh_max = (hin, hout,dt)
        if max_hh[id_hh_max] > 0:
            max_constraint = lpSum(lpvar_flow_hh[hin,hout,t,dt]
                                   for t in tranches) <= max_hh[id_hh_max]
            prob += max_constraint, 'ArcMax_%s' % '_'.join(id_hh_max)
        # else: 
           # nothing - ignore zero max flows for efficiency
    ...
    for dt in stry_dates:
        ...
        for h in hubs:
            ...
            prob += lpSum(hflows_sh) + lpSum(hflows_in_hh)+ lpSum(hflows_ext_sto)\
                == lpSum(hflows_hd) + lpSum(hflows_out_hh) + lpSum(hflows_inj_sto), 'HMBC_%s_%s' % (h,dt)
...
# loop to next year
    #return results
    return solved_supply, solved_prices_new, solved_flows, solver_info, solved_gas_invt, solved_gas_injected, solved_gas_extracted
