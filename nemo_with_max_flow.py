...
directory = os.getcwd()
os.chdir(directory)
def run_model_max(supplycap,supplycost,arccap,arccost,arcmin,arcmax,dmd,tariff_surc):
    ...
    # PROCESS
    tStart = dtm.now()
    results = solve_network(supply, arcs, arc_min_flow,arc_max_flow, demand, actual_dates)
    tEnd = dtm.now()
...
def solve_network(supply, arcs, arc_min_flow,arc_max_flow, demand, dates):
    """
    ...
    for date in dates:  # for each month
        ...
        # arc_max_flow_Winnie
        arc_max_flow_m = arc_max_flow[arc_max_flow['date'] == date].copy()
        ...
        # max flow_Winnie
        max_hh = arc_max_flow_m.set_index(['from_hub', 'to_hub'])[
                'max_flow'].to_dict()  # maxflow doesn't have tranche 
        
        for (hin, hout) in max_hh:
            id_hh = (hin, hout)
            if max_hh[id_hh] > 0:
                max_constraint = lpSum(lpvar_flow_hh[hin][hout][t]
                                       for t in tranches) <= max_hh[id_hh]
                prob += max_constraint, 'ArcMax_%s' % '_'.join(id_hh)
            # else: 
