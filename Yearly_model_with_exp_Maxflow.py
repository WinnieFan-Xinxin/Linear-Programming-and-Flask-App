...
directory = os.getcwd()
os.chdir(directory)
def run_model_exp_max(supplycap,supplycost,arccap,arccost,arcmin,arcmax,dmd,tariff_surc,
    ...
    dflst = [supply,arcs,arc_min_flow,arc_max_flow,demand,export]
    for df in dflst:
        df = df.sort_values(['date']).copy()

    #--------------------------------------------------------------------------------

    # by this point we have all our input data (supply, arcs, minflows, maxflow, demand, export)
    ...
    if Forward12m == False:
        ...
        for i in range(0,len(actual_dates),12):
            ...
            solution = solve_network(ID,d_range,supply,arcs,arc_min_flow,arc_max_flow,demand,export)
        ...
        for date in act_dates:
            ...
            if date != act_dates[-1]:
                ...
                solution = solve_network(dateID,dateRange,supply,arcs,arc_min_flow,arc_max_flow,demand,export)
            ...
            # keep the solution for the whole year
                ...
                last_solve = solve_network(dateID,dateRange,supply,arcs,arc_min_flow,arc_max_flow,demand,export)
...
def solve_network(dateID,dates,supply,arcs,arc_min_flow,arc_max_flow,demand,export):
    """
